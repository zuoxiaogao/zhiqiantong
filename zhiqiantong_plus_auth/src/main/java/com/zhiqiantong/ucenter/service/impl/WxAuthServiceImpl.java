...
public class WxAuthServiceImpl implements AuthService, WxAuthService {
    ...
    public ZqtUserExt execute(AuthParamsDto authParamsDto) {
        ...
        ZqtUser user = zqtUserMapper.selectOne(new LambdaQueryWrapper<ZqtUser>().eq(ZqtUser::getUsername, username));
        if(user==null){
            //返回空表示用户不存在
    ...
    @Override
    public ZqtUser wxAuth(String code) {
    ...
    @Transactional
    public ZqtUser addWxUser(Map userInfo_map){
        String unionid = userInfo_map.get("unionid").toString();
        //根据unionid查询数据库
        ZqtUser zqtUser = zqtUserMapper.selectOne(new LambdaQueryWrapper<ZqtUser>().eq(ZqtUser::getWxUnionid, unionid));
        if(zqtUser!=null){
            return zqtUser;
        }
        String userId = UUID.randomUUID().toString();
        zqtUser = new ZqtUser();
        zqtUser.setId(userId);
        zqtUser.setWxUnionid(unionid);
        //记录从微信得到的昵称
        zqtUser.setNickname(userInfo_map.get("nickname").toString());
        zqtUser.setUserpic(userInfo_map.get("headimgurl").toString());
        zqtUser.setName(userInfo_map.get("nickname").toString());
        zqtUser.setUsername(unionid);
        zqtUser.setPassword(unionid);
        zqtUser.setUtype("101001");//学生类型
        zqtUser.setStatus("1");//用户状态
        zqtUser.setCreateTime(LocalDateTime.now());
        zqtUserMapper.insert(zqtUser);
        ZqtUserRole zqtUserRole = new ZqtUserRole();
        zqtUserRole.setId(UUID.randomUUID().toString());
        zqtUserRole.setUserId(userId);
        zqtUserRole.setRoleId("17");//学生角色
        zqtUserRoleMapper.insert(zqtUserRole);
        return zqtUser;
    }
